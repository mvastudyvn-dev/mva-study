...
async function scrape() {
  try {
    ...
    // We need to find the majors inside pageProps
    let majorsList = [];
    
    // It's probably in pageProps.initialState or similar
    function findMajors(obj) {
        if (Array.isArray(obj)) {
            for (let i=0; i<obj.length; i++) {
                if (obj[i] && typeof obj[i] === 'object' && obj[i].code_list && obj[i].name && obj[i].alias) {
